//! Running operations on the open repository, and asking the user for credentials git needs.
//!
//! `run_operation` resolves with the outcome; progress arrives meanwhile as `opProgress` events for
//! the operation's id. When git needs a password, token or passphrase, a `credentialPrompt` event
//! asks the UI, which replies with `answer_prompt`.
...
pub async fn run_operation(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let mut progress = |text: &str| {
            ...
            let _ = OpProgress { id: progress_id.clone(), text: text.to_owned() }.emit(&app);
        };
