import type { OpError, OpOutcome, Operation, RepoInfo } from "./bindings";
...
/**
 * Runs operations on one tab's repository and keeps what the UI shows about them: the ones on their
...
export class Operations {
  ...
  #client: Pick<RepoClient, "runOperation">;
  #adopt: (info: RepoInfo) => void;
  #inFlight = new Map<string, Promise<OpOutcome>>();

  constructor(client: Pick<RepoClient, "runOperation">, adopt: (info: RepoInfo) => void) {
    this.#client = client;
    this.#adopt = adopt;
  }
   ...
   * only if the backend can't run operations at all (the tab's session is gone), which the app-wide
  ...
  /** Shows a progress line git reported for operation `id`; the app routes each to its tab. */
