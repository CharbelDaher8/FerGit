...
/** What the finder moves around in: the view of its tab's repository. `RepoView` is one. */
...
export class Finder {
  ...
  readonly #client: Pick<RepoClient, "search">;

  /** Searches the repository of `client`, moving around in `target`. */
  constructor(target: () => FindTarget | null, client: Pick<RepoClient, "search">) {
    this.#target = target;
    this.#client = client;
  }
