...
// dialogs and to the tab the action is for.
...
/** The repository an action is carried out on: a tab's operations, and its client for undo. */
export interface ActionTarget {
  operations: Operations;
  client: Pick<RepoClient, "undoable">;
}
...
/**
 * Asks for whatever `action` needs, then runs it on `target`, the tab it was picked in, even if
 * another tab is in front by the time the dialogs are answered. Cancelling a dialog runs nothing.
 */
export async function perform(target: ActionTarget, action: MenuAction): Promise<void> {
  if (action.kind === "undo") return undo(target);
  ...
  await target.operations.run(op, title);
}

/** Says what undo would do (or why it can't), and does it once confirmed. */
async function undo({ operations, client }: ActionTarget): Promise<void> {
  const undoable = await client.undoable();
